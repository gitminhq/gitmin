...
func printBanner() {
	myFigure := figure.NewColorFigure("Gitmin", "", "blue", true)
	myFigure.Print()
}
...
func main() {
	...
	application := app.New(
		...
		app.WithHTMLTemplateDir("./templates"),
		app.WithStaticAssetDir("./static"),
	)

	routers.RegisterRoutes(application)
